...
- 設定 (暗号化 config): チューニング値 (和解/二次被害/カルマ/カード/経済/政治/演出…) と秘密 (VAPID) は **単一の暗号化 config** に集約する (旧 `PAGUS_*` env ~70 個を統合、SecretBox=AES-256-GCM を Concordia から流用 `server/src/config/`)。`pnpm pagus:config init` で `data/runtime/pagus.config.enc` を既定値から作成し、`pnpm pagus:config set <dotpath> <value>`(例 `karma.rate 0.9`)/`show`/`import <file.json>` で編集する。鍵は `data/runtime/pagus.config.key` (リポ外扱い・gitignore) か env passphrase `PAGUS_CONFIG_KEY`。enc も鍵もコミットしない。スキーマ参考は平文 `data/config/pagus.config.example.json`。config ファイルが無ければ既定値で起動し warn する (復号/型不正は fail-fast で throw)。
  - env のまま残す例外 (launch/operational): `PAGUS_CONFIG_KEY`(マスター鍵) / `PAGUS_FRESH`(その起動だけ world.json 無視) / `PAGUS_BRAIN`(stub|llm 起動モード) / `PAGUS_DATA_DIR`(config 自体の置き場解決)。
