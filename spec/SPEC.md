...
### 8A.1 村の評判ベクトル (6 軸レーダー)

- 村は **6 軸の評判ベクトル**を持ち、レーダーチャートで表示する。<6 軸の定義は確定要 → §11>。
- 各どうぶつも同じ 6 軸の**個体ベクトル**を持つ。
...
- **評判 6 軸の定義** (各軸の名前と意味) は**未確定** → ユーザ確認待ち。村ベクトル・個体ベクトル・グループ・出生バイアスの全てがこれに依存。
