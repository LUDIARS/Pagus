...
- **永続化(runtime, gitignore)**: ログ JSONL (`logs/pagus-*.jsonl`) / 糾弾プール (`data/runtime/denunciations.json`) / 村の歴史 (`data/runtime/chronicle.json`)。
...
| v0.6 | 人口増減バランス調整 + **world スナップショット永続化(未)** + push通知投票(未) + 祝日イベント実発火(未) |

> runtime 永続化は糾弾プール/村の歴史のみ実装済。world 全体 (どうぶつ状態) のスナップショット保存・復元は未 (§11)。
...
> **残る未確定** — world スナップショット永続化形式(JSON/SQLite)、変動祝日の正確日(現状は近似)、祝日イベントの実発火、睡眠干渉の対象指定経路、気質→徳目1:1の妥当性(特に野心→信仰)、各メカニクスの数値バランス(観戦して調整)。
