...
| v0.6 | 人口増減バランス調整 + **world スナップショット永続化 ✅** + push通知投票(未) + 祝日イベント実発火(未) |
...
> **2026-06-26 追加で確定** — world スナップショット永続化形式 = **JSON** (`data/runtime/world.json`、糾弾プール/村の歴史と同じ持ち方に統一。`toWire`/`fromWire` で Map を配列に往復。SQLite は不採用 — 単一プロセス権威・小規模・JSON で十分)。
> **残る未確定** — 変動祝日の正確日(現状は近似)、祝日イベントの実発火、睡眠干渉の対象指定経路、気質→徳目1:1の妥当性(特に野心→信仰)、各メカニクスの数値バランス(観戦して調整)。
