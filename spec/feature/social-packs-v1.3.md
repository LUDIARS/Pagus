...
> 2026-06-28 起草。ユーザ採択 (4パック全採用) を正本化。既存資産 (カルマ/善性/stats/陣営=`server/player-state.ts`、村のルール/BehaviorRule/裁判=sim) の上に**毎 tick LLM を増やさず**乗せる。数値は env 可変・観戦調整前提。
>
...
- **裁判**は介入メニューから分離し、裁判中だけ中央ステージ上の専用 `trial-panel` に出す。内容は「誰を裁くか」→「死刑/教育」。
- カルマ残高・クールダウン・コストを常時表示し、不可操作は無効化＋理由表示 (commandRejected と整合)。
- 既存の散在 UI (trial-panel/leaderboard/status/chronicle 等) をこのオーバーレイ配下へ再編。行動ドック以外はモバイルでドロワー。
