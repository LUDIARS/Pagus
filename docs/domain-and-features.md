...
観客(プレイヤー)は村の外から扇動・制裁・応援・裁判参加で介入し、村の変容を集団で楽しむ。
...
userId ごとのカルマ (時間経過で自動蓄積) と善性 (virtue)。操作 = **扇動** (対象 1 体 + 偽の噂注入、カルマ消費) / **制裁** (当日つるし上げ裁判、善性に比例して高額) / **応援** (対象パラメータ+、3 分インターバル)。**推し指名** (生存中カルマ加速、死で損失 + 村に追悼のしきたりが残る = 弔い。推しの感情・情報・行動特性を歪めて村へ波及させる間接洗脳の主経路) / **裁判参加** (誰を裁くか、死刑か教育かを選ぶ) / **称号** (破壊神/聖人/立法者/陪審員…) / **二大陣営** (善導 vs 扇動の徳目綱引き)。

- 実装: `server/src/player-state.ts`, `sim/src/term-machine.ts` (`inciteTarget`/`sanction`/`cheer`/`resolveTrialAfterPlayerStatement`)
...
| F24 | 裁判参加 (誰を裁くか / 死刑か教育か) | D10 | ✅ v1.2-B | SPEC §13.3 |
