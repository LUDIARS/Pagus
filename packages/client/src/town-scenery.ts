import { townMap, townRoute, townSite, type WorldConfig } from '@pagus/sim';
...
export function townScenery(config: WorldConfig, damagedHomes: ReadonlySet<string> = new Set()): ShapePart[] {
  ...
  for (const site of map.sites) {
    ...
    parts.push(...townBuilding(site, townPoint(config, site.position), halfCell, damagedHomes.has(site.id)));
  }
